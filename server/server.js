...
exports = {
  onTicketUpdateCallback: async function (args) {
    ...
    const freshserviceTicketId = ticket.id;
    ...
    const syncFieldChanges = ticket.changes?.custom_fields?.freshdesk_sync;
    const syncValue = Array.isArray(syncFieldChanges) && syncFieldChanges[1] === 'true';
    ...
    try {
      ...
      console.log("Freshdesk ticket created successfully. ID:", freshdeskTicketId);
    } catch (error) {
    ...
    try {
      const response = await $request.invokeTemplate("updateFsTicketCustomField", {
        iparam: {
          ...
          ticket_id: freshserviceTicketId
        },
